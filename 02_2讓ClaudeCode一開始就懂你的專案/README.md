# 單元2 - 讓ClaudeCode一開始就懂你的專案

## 專案設定

自[example](https://github.com/uopsdod/claude_code_treasure_game/tree/initial)下載zip。


## 開始實作

### 01-建立CLAUDE.md讓ClaudeCode了解專案的運作方式

```bash
/init
```

### 02-指定檔案-用正確Context取代冗長的Prompt
...
**練習**
題目: 使用@src/audios/chest_open_with_evil_laugh.mp3來取代@src/audios/chest_open.mp3的聲音效果，讓打開寶箱的聲音變成有骷髏頭裡面發出邪惡笑聲的聲音效果。
...
### 03-把視覺納入開發流程-用截圖來直覺式開發

1. 截圖並標記你想要修改的區域。
2. 複製截圖並貼在對話框裡(ubuntu cli有問題, 需要在VSCODE中的ClaudeCode extension裡操作)

```text
show the results to be either: win, tie, loss in the circled place according to the final score
```

**練習**
在滑鼠懸停在關閉的寶箱上時，使用src/assets/key.png圖示來取代原本的鼠標指標。
